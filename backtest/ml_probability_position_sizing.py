...
SIZING_METHODS = ["Binary", "ThreeStep", "FourStep"]
...
def create_features(price_df: pd.DataFrame) -> pd.DataFrame:
    """Create technical features and 5D +1% target."""
    df = price_df.copy()
...
def apply_regime_filter(df: pd.DataFrame, position: pd.Series, regime_filter: str) -> pd.Series:
    """Apply moving-average regime filter to position size."""
    if regime_filter == "NO_FILTER":
        return position
    if regime_filter == "MA100":
        return position.where(df["close"] > df["ma100"], 0.0)
    if regime_filter == "MA150":
        return position.where(df["close"] > df["ma150"], 0.0)
    if regime_filter == "MA200":
        return position.where(df["close"] > df["ma200"], 0.0)

    raise ValueError(f"Unknown regime filter: {regime_filter}")


def build_position(probability: pd.Series, base_threshold: float, sizing_method: str) -> pd.Series:
    """Convert model probability into position size."""
    if sizing_method == "Binary":
        return (probability >= base_threshold).astype(float)

    if sizing_method == "ThreeStep":
        position = pd.Series(0.0, index=probability.index)
        position[(probability >= base_threshold) & (probability < base_threshold + 0.05)] = 0.5
        position[probability >= base_threshold + 0.05] = 1.0
        return position

    if sizing_method == "FourStep":
        position = pd.Series(0.0, index=probability.index)
        position[(probability >= base_threshold) & (probability < base_threshold + 0.05)] = 0.3
        position[(probability >= base_threshold + 0.05) & (probability < base_threshold + 0.10)] = 0.6
        position[probability >= base_threshold + 0.10] = 1.0
        return position

    raise ValueError(f"Unknown sizing method: {sizing_method}")


def backtest_position(df: pd.DataFrame, position: pd.Series):
    """Backtest fractional position with one-day execution delay and transaction costs."""
    ...
    return net_return, turnover, cost_drag, avg_exposure, max_exposure
...
def main():
    asset_rows = []
    benchmark_returns = {}
    strategy_returns = {method: {} for method in SIZING_METHODS}
    ...
    for asset_name, ticker in ASSETS.items():
        ...
        for sizing_method in SIZING_METHODS:
            print(f"Testing sizing method: {sizing_method}")
            ...
            for threshold in THRESHOLDS:
                for regime_filter in REGIME_FILTERS:
                    valid_position = build_position(valid_prob, threshold, sizing_method)
                    valid_position = apply_regime_filter(valid_df, valid_position, regime_filter)

                    valid_returns, _, _, _, _ = backtest_position(valid_df, valid_position)
            ...
            test_position = build_position(test_prob, best_config["threshold"], sizing_method)
            test_position = apply_regime_filter(test_df, test_position, best_config["regime_filter"])

            test_returns, turnover, cost_drag, avg_exposure, max_exposure = backtest_position(
                test_df,
                test_position,
            )

            test_metrics = calculate_metrics(test_returns)
            strategy_returns[sizing_method][asset_name] = test_returns
    ...
    asset_summary.to_csv(RESULT_DIR / "ml_probability_sizing_asset_summary.csv", index=False)
    ...
    for portfolio_name, asset_list in portfolio_sets.items():
        ...
        for sizing_method, returns_by_asset in strategy_returns.items():
            model_df = pd.concat(
            ...
            equity_curves[f"{portfolio_name} {sizing_method}"] = (1 + model_portfolio_returns).cumprod()

    portfolio_summary = pd.DataFrame(portfolio_rows)
    portfolio_summary.to_csv(RESULT_DIR / "ml_probability_sizing_portfolio_summary.csv", index=False)
    ...
    plt.title("Day 30 - Probability-Based Position Sizing: Robust Assets Portfolio")
    plt.xlabel("Date")
    plt.ylabel("Equity Curve")
    plt.legend()
    plt.tight_layout()
    plt.savefig(CHART_DIR / "probability_sizing_robust_assets_equity_curves.png", dpi=150)
    plt.close()

    print("\n==============================")
    print("Asset-Level Probability Position Sizing Test")
    print("==============================")
    ...
    print("\n==============================")
    print("Portfolio-Level Probability Position Sizing Test")
    print("==============================")
    ...
    print("\nSaved files:")
    print(f"- {RESULT_DIR / 'ml_probability_sizing_asset_summary.csv'}")
    print(f"- {RESULT_DIR / 'ml_probability_sizing_portfolio_summary.csv'}")
    print(f"- {CHART_DIR / 'probability_sizing_robust_assets_equity_curves.png'}")
