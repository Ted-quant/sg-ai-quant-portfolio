...
def get_models() -> dict:
    """Return model dictionary for ensemble testing."""
    return {
...
def main():
    ...
    for asset_name, ticker in ASSETS.items():
        ...
        for model_name, model in models.items():
            ...
            valid_probabilities[model_name] = pd.Series(
                model.predict_proba(X_valid)[:, 1],
                index=valid_df.index,
            )

            test_probabilities[model_name] = pd.Series(
                model.predict_proba(X_test)[:, 1],
                index=test_df.index,
            )

        ensemble_probs = {
            "ExtraTrees": test_probabilities["ExtraTrees"],
            "EqualWeightEnsemble": pd.concat(test_probabilities.values(), axis=1).mean(axis=1),
            "Top3Ensemble": pd.concat(
                [
                    test_probabilities["ExtraTrees"],
        ...
        valid_ensemble_probs = {
            "ExtraTrees": valid_probabilities["ExtraTrees"],
            "EqualWeightEnsemble": pd.concat(valid_probabilities.values(), axis=1).mean(axis=1),
            "Top3Ensemble": pd.concat(
                [
                    valid_probabilities["ExtraTrees"],
                    valid_probabilities["RandomForest"],
                    valid_probabilities["GradientBoosting"],
                ],
                axis=1,
            ).mean(axis=1),
        }
        ...
        for strategy_name in ["ExtraTrees", "EqualWeightEnsemble", "Top3Ensemble"]:
            best_score = -np.inf
            best_config = None
            ...
            for threshold in THRESHOLDS:
                for regime_filter in REGIME_FILTERS:
                    valid_signal = build_signal(
                        valid_df,
                        valid_ensemble_probs[strategy_name],
                        threshold,
                        regime_filter,
                    )
            ...
            strategy_returns[strategy_name][asset_name] = test_returns
    ...
    asset_summary.to_csv(RESULT_DIR / "ml_ensemble_signal_asset_summary.csv", index=False)
    ...
    for portfolio_name, asset_list in portfolio_sets.items():
        ...
        for strategy_name, returns_by_asset in strategy_returns.items():
            model_df = pd.concat(
            ...
            equity_curves[f"{portfolio_name} {strategy_name}"] = (1 + model_portfolio_returns).cumprod()

    portfolio_summary = pd.DataFrame(portfolio_rows)
    portfolio_summary.to_csv(RESULT_DIR / "ml_ensemble_signal_portfolio_summary.csv", index=False)
    ...
    plt.title("Day 27 - Ensemble Signal Test: Robust Assets Portfolio")
    plt.xlabel("Date")
    plt.ylabel("Equity Curve")
    plt.legend()
    plt.tight_layout()
    plt.savefig(CHART_DIR / "ensemble_signal_robust_assets_equity_curves.png", dpi=150)
    plt.close()

    print("\n==============================")
    print("Asset-Level Ensemble Signal Test")
    print("==============================")
    ...
    print("\n==============================")
    print("Portfolio-Level Ensemble Signal Test")
    print("==============================")
    ...
    print("\nSaved files:")
    print(f"- {RESULT_DIR / 'ml_ensemble_signal_asset_summary.csv'}")
    print(f"- {RESULT_DIR / 'ml_ensemble_signal_portfolio_summary.csv'}")
    print(f"- {CHART_DIR / 'ensemble_signal_robust_assets_equity_curves.png'}")
