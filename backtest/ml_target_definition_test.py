...
def download_price_data(ticker: str) -> pd.DataFrame:
    """Download adjusted close data from yfinance with simple retry logic."""
    last_error = None
...
def create_base_features(price_df: pd.DataFrame) -> pd.DataFrame:
    """Create technical features used by all target definitions."""
    df = price_df.copy()
...
def add_target(df: pd.DataFrame, horizon: int, threshold: float) -> pd.DataFrame:
    """Add target based on forward return horizon and threshold."""
    out = df.copy()
    out["forward_return"] = out["close"].shift(-horizon) / out["close"] - 1
    out["target"] = (out["forward_return"] > threshold).astype(int)
    return out.dropna()
...
def main():
    asset_rows = []
    benchmark_returns = {}
    strategy_returns = {target_name: {} for target_name in TARGET_DEFINITIONS.keys()}

    base_data = {}

    for asset_name, ticker in ASSETS.items():
        print(f"\nDownloading and preparing {asset_name} ({ticker})...")
        price_df = download_price_data(ticker)
        base_data[asset_name] = create_base_features(price_df)

    for asset_name, base_df in base_data.items():
        print(f"\nProcessing {asset_name}...")

        benchmark_df = base_df.dropna().loc[TEST_START:].copy()
        benchmark_returns[asset_name] = benchmark_df["return_1d"]
        benchmark_metrics = calculate_metrics(benchmark_df["return_1d"])

        for target_name, target_config in TARGET_DEFINITIONS.items():
            print(f"Testing target definition: {target_name}")
            ...
            strategy_returns[target_name][asset_name] = test_returns

            asset_rows.append({
                "asset": asset_name,
                "target": target_name,
                "prediction_horizon": target_config["horizon"],
                "target_return_threshold": target_config["threshold"],
                "selected_signal_threshold": best_config["signal_threshold"],
    ...
    asset_summary.to_csv(RESULT_DIR / "ml_target_definition_asset_summary.csv", index=False)
    ...
    for portfolio_name, asset_list in portfolio_sets.items():
        ...
        for target_name, returns_by_asset in strategy_returns.items():
            model_df = pd.concat(
            ...
            equity_curves[f"{portfolio_name} {target_name}"] = (1 + model_portfolio_returns).cumprod()

    portfolio_summary = pd.DataFrame(portfolio_rows)
    portfolio_summary.to_csv(RESULT_DIR / "ml_target_definition_portfolio_summary.csv", index=False)
    ...
    plt.title("Day 29 - Target Definition Test: Robust Assets Portfolio")
    plt.xlabel("Date")
    plt.ylabel("Equity Curve")
    plt.legend()
    plt.tight_layout()
    plt.savefig(CHART_DIR / "target_definition_robust_assets_equity_curves.png", dpi=150)
    plt.close()

    print("\n==============================")
    print("Asset-Level Target Definition Test")
    print("==============================")
    ...
    print("\n==============================")
    print("Portfolio-Level Target Definition Test")
    print("==============================")
    ...
    print("\nSaved files:")
    print(f"- {RESULT_DIR / 'ml_target_definition_asset_summary.csv'}")
    print(f"- {RESULT_DIR / 'ml_target_definition_portfolio_summary.csv'}")
    print(f"- {CHART_DIR / 'target_definition_robust_assets_equity_curves.png'}")
